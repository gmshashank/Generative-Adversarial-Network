...
class DCGAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 1:
            ...
            loss_disc_real = self.adversarial_loss(disc_real,
                    torch.ones_like(disc_real))
            disc_fake = self.discriminator(fake.detach()).reshape(-1)
            loss_disc_fake = self.adversarial_loss(disc_fake,
    ...
    def setup(self, stage: str):
        ...
        train,valid = torch.utils.data.random_split( original_dataset, [split_a_size, split_b_size], generator=torch.Generator().manual_seed(MANUAL_SEED))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--latent_dim", type=int, default=100, help="dimensionality of latent space"
    )
